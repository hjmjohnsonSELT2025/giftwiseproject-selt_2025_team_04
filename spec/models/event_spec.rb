...
RSpec.describe Event, type: :model do
  it "should add events" do
    @event = Event.new({:title => "test1", :description => "test event", :location => "test location", :owner_id => User.find_by(:email => "a@b.c").id})
  ...
  it "should reference a user" do
    @event = Event.new({:title => "test1", :description => "test event", :location => "test location", :owner_id => User.find_by(:email => "a@b.c").id})
    expect(@event.owner.email).to eq("a@b.c")
  end
  ...
  it "should add recipients" do
    ...
    @event = Event.new({:title => "test event title", :description => "test event", :location => "test location", :owner_id => User.find_by(:email => "a@b.c").id})
